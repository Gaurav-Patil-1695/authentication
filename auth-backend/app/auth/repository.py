...
async def set_user_active(
    conn: AsyncConnection, user_id: int, is_active: bool
) -> None:
    await conn.execute(
        text(
            """
...
async def get_password_reset_by_token_hash(
    ...
    result = await conn.execute(
        text("SELECT * FROM password_resets WHERE token_hash = :token_hash"),
        {"token_hash": token_hash},
    )
    row = result.mappings().first()
    return dict(row) if row else None
...
async def invalidate_password_resets_for_user(
    ...
    """Mark all unused resets for a user as used (e.g. after a successful reset)."""
    await conn.execute(
        text(
            """
            UPDATE password_resets
...
async def revoke_all_refresh_tokens_for_user(
    ...
    """Revoke all active refresh tokens for a user (e.g. on logout-all or password change)."""
    await conn.execute(
        text(
            """
            UPDATE refresh_tokens
...
async def update_refresh_token_hash(
    ...
    """Rotate a refresh token: replace its hash and expiry in-place."""
    result = await conn.execute(
        text(
            """
            UPDATE refresh_tokens
